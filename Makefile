...
install_tinymce4:
	...
	( wget -c -O /tmp/tmce4.zip http://download.ephox.com/tinymce/community/tinymce_4.3.7.zip && unzip -qo /tmp/tmce4.zip -d /tmp && cd /tmp/tinymce && find . -type f -exec $(SUDO) install -m 664 -g devel -D {} $(QLIB)/j/tinymce4/{} \; )
...
install_bootstrap3:
	( if [ ! -d "$(QLIB)/j/bootstrap" ]; then ($(SUDO) mkdir $(QLIB)/j/bootstrap && $(SUDO) chmod g+w $(QLIB)/j/bootstrap && $(SUDO) chown :devel $(QLIB)/j/bootstrap); fi )
	( rm -rf /tmp/bootstrap-$(BOOTSTRAP)-dist* /tmp/dist)
	( wget -c -O /tmp/bootstrap-$(BOOTSTRAP)-dist.zip https://github.com/twbs/bootstrap/releases/download/v$(BOOTSTRAP)/bootstrap-$(BOOTSTRAP)-dist.zip && unzip -qo /tmp/bootstrap-$(BOOTSTRAP)-dist.zip -d /tmp && cd /tmp/bootstrap-$(BOOTSTRAP)-dist && find . -type f -exec $(SUDO) install -m 664 -g devel -D {} $(QLIB)/j/bootstrap/{} \; )
...
#	(wget --no-check-certificate -c -O /tmp/jstools-qui/typeahead.jquery.js      https://raw.githubusercontent.com/twitter/typeahead.js/master/dist/typeahead.jquery.js)
	...
	(cd /tmp/jstools-qui && find . -type f -exec $(SUDO) install -m 664 -g devel -D {} $(QLIB)/j/{} \; )
...
install_bootstrap_datepicker:
	...
	(cd /tmp/bsdp && find . -type f -exec $(SUDO) install -m 664 -g devel -D {} $(QLIB)/j/bootstrap-datepicker/{} \; )

install_js_sprintf:
	( if [ ! -d /tmp/jssp ]; then mkdir /tmp/jssp; fi )
	(cd /tmp/jssp && wget -c https://github.com/alexei/sprintf.js/archive/$(SPRINTF_JS).tar.gz)
	(cd /tmp/jssp && tar -xzf $(SPRINTF_JS).tar.gz && $(SUDO) install -m 664 -g devel -D sprintf.js-$(SPRINTF_JS)/dist/sprintf.min.js $(QLIB)/j/sprintf.min.js )
...
install_snap:
	...
	(cd /tmp/jssnap && $(SUDO) install -m 664 -g devel -D Snap.svg-$(SNAP)/dist/snap.svg.js      $(QLIB)/j/ )
	(cd /tmp/jssnap && $(SUDO) install -m 664 -g devel -D Snap.svg-$(SNAP)/dist/snap.svg-min.js  $(QLIB)/j/ )
...
#	( if [ ! -d $(QLIB)/j/tinymce4/js/tinymce/plugins/youtube/ ]; then $(SUDO) mkdir $(QLIB)/j/tinymce4/js/tinymce/plugins/youtube/; fi )
	(cd /tmp/tmyout/ && find youtube -type f -exec $(SUDO) install -m 664 -g devel -D {} $(QLIB)/j/tinymce4/js/tinymce/plugins/{} \; )



install_templates:
	( if [ ! -d "$(QUI-CTPP)" ]; then ($(SUDO) mkdir -p $(QUI-CTPP) && $(SUDO) chmod g+w $(QUI-CTPP) && $(SUDO) chown :devel $(QUI-CTPP)); fi )
	( cd ctpp; for file in `find . -type f`; do $(SUDO) install -m 664 -g devel -D $$file $(QUI-CTPP)/$$file; done; )

install_static:
	( if [ ! -d "$(QUI-HTDOCS)" ]; then ($(SUDO) mkdir -p $(QUI-HTDOCS) && $(SUDO) chmod g+w $(QUI-HTDOCS) && $(SUDO) chown :devel $(QUI-HTDOCS)); fi)
	( cd htdocs; for file in `find . -type f `; do $(SUDO) install -m 664 -g devel -D $$file $(QUI-HTDOCS)/$$file; done; )
